...
- *Interactive* --- Code copy buttons, language labels, active-heading TOC tracking, and horizontally scrollable tables (progressive enhancement)
...
Run all checks --- this is the single entry point for testing:

#+BEGIN_SRC shell
nix flake check
#+END_SRC

=nix flake check= runs the complete suite with no other tooling required:

| Check     | What it verifies                                          |
|-----------+-----------------------------------------------------------|
| =build=   | Every theme bundles (CSS + JS + setup file)               |
| =lint=    | All CSS parses against the browser target (lightningcss)  |
| =js=      | The runtime JS bundles and minifies cleanly (esbuild)     |
| =setup=   | Generated setup files have no unsubstituted placeholders  |
| =html=    | Exported HTML is well-formed and wires up the assets      |
| =example= | The example documents export with the default theme       |
| =site=    | The full multi-theme demo site assembles                  |
